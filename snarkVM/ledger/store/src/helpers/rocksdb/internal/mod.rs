...
/// Default block cache for production ledger databases (8 GiB).
const DEFAULT_LEDGER_BLOCK_CACHE_GIB: f64 = 8.0;
/// Safety cap for misconfigured environments (512 GiB).
const MAX_LEDGER_BLOCK_CACHE_GIB: f64 = 512.0;
const MIB: usize = 1024 * 1024;
const GIB: usize = 1024 * MIB;

/// Block cache size for unit tests and ephemeral `StorageMode::Test` stores.
const TEST_LEDGER_BLOCK_CACHE_BYTES: usize = 64 * MIB;

/// Returns the RocksDB block cache capacity for the given storage mode.
///
/// Production and development ledgers honor `SNARKOS_ROCKSDB_BLOCK_CACHE_GB` (gibibytes, fractional
/// allowed). When unset, [`DEFAULT_LEDGER_BLOCK_CACHE_GIB`] is used.
fn ledger_block_cache_capacity_bytes(storage: &StorageMode) -> usize {
    if matches!(storage, StorageMode::Test(_)) {
        return TEST_LEDGER_BLOCK_CACHE_BYTES;
    }

    let gib = match std::env::var("SNARKOS_ROCKSDB_BLOCK_CACHE_GB") {
        Ok(raw) => match raw.trim().parse::<f64>() {
            Ok(value) if value.is_finite() => value,
            _ => {
                warn!(
                    "Invalid SNARKOS_ROCKSDB_BLOCK_CACHE_GB={raw:?}; using default {DEFAULT_LEDGER_BLOCK_CACHE_GIB} GiB"
                );
                DEFAULT_LEDGER_BLOCK_CACHE_GIB
            }
        },
        Err(_) => DEFAULT_LEDGER_BLOCK_CACHE_GIB,
    };

    let gib = gib.clamp(0.0, MAX_LEDGER_BLOCK_CACHE_GIB);

    if gib <= f64::EPSILON {
        warn!("SNARKOS_ROCKSDB_BLOCK_CACHE_GB is zero; using minimum block cache size (8 MiB)");
        return 8 * MIB;
    }

    let bytes_f = (gib * GIB as f64).round();
    if !bytes_f.is_finite() || bytes_f <= 0.0 {
        return 8 * MIB;
    }
    let capped = bytes_f.min(usize::MAX as f64);
    (capped as usize).max(8 * MIB)
}
...
pub struct RocksDB {
    ...
    /// Keeps the block cache alive for the lifetime of the database (see [`rocksdb::Cache`]).
    block_cache: Arc<rocksdb::Cache>,
    /// The network ID.
    network_id: u16,
    /// The storage mode.
...
impl Database for RocksDB {
    ...
    fn open<S: Into<StorageMode>>(network_id: u16, storage: S) -> Result<Self> {
        ...
        let database = if let Some(db) = databases.get(&db_path) {
            ...
            let cache_capacity = ledger_block_cache_capacity_bytes(&storage);
            let block_cache = Arc::new(rocksdb::Cache::new_lru_cache(cache_capacity));

            let mut block_based_options = rocksdb::BlockBasedOptions::default();
            block_based_options.set_block_cache(&block_cache);
            // With a large block cache, caching index and filter blocks reduces read amplification.
            ...
            // Spread levels across byte ranges based on actual data size (RocksDB recommended default).
