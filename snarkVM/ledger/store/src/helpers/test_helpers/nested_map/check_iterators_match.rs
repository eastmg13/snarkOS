...
fn check_pending_iterator(map: &impl for<'a> NestedMap<'a, usize, usize, String>, expected_length: usize) {
    let mut counter = 0;
...
fn check_confirmed_iterators_match(map: &impl for<'a> NestedMap<'a, usize, usize, String>, expected_length: usize) {
    let mut counter = 0;
    ...
    while counter < expected_length {
        let (map_0, key_0, value_0) = iter_confirmed.next().unwrap();
        let (map_1, key_1) = keys_confirmed.next().unwrap();
...
pub fn check_iterators_match(map: impl for<'a> NestedMap<'a, usize, usize, String>) {
